...
function App() {
  ...
  useEffect(() => {
    fetch("http://books-app.local:8000/books")
      .then((res) => res.json())
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://books-app.local:8000/books/add", {
        method: "POST",
  ...
  const handleDelete = async (e) => {
    try {
      await fetch("http://books-app.local:8000/books/delete/" + e.target.id, { method: "DELETE" });
